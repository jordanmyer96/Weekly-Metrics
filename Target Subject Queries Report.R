...
filteredImportQueries <- importQueries %>% filter(SUBJECT.ID%in%importFirstBatch$Subject.ID)
openQueries <- filteredImportQueries%>%
  filter(STATUS=="Open")
openWithoutAdj <- filteredImportQueries%>%
  filter(STATUS=="Open")%>%
...
answeredQueriesDF <- filteredImportQueries %>% 
...
wbName <- paste("../11.3.4 Output Files/Outgoing Weekly Reports/Query Reports/Target Subject Queries.xlsx",sep = "")
